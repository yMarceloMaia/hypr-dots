        ...
        Column {
            ...
            Grid {
                ...
                Tile {
                    ...
                    label: "CPU"
                    active: root.modCpu
                    onActivated: root.modCpu = !root.modCpu
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Volume"
                    active: root.modVolume
                    onActivated: root.modVolume = !root.modVolume
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Weather"
                    active: root.modWeather
                    onActivated: root.modWeather = !root.modWeather
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Battery"
                    active: root.modBattery
                    onActivated: root.modBattery = !root.modBattery
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Clock"
                    active: root.modClock
                    onActivated: root.modClock = !root.modClock
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Workspace"
                    active: root.modWorkspace
                    onActivated: root.modWorkspace = !root.modWorkspace
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Screen Rec."
                    active: root.modScreenRecord
                    onActivated: root.modScreenRecord = !root.modScreenRecord
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Notifications"
                    active: root.modNotification
                    onActivated: root.modNotification = !root.modNotification
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Media"
                    active: root.modMpris
                    onActivated: root.modMpris = !root.modMpris
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Updates"
                    active: root.modArch
                    onActivated: root.modArch = !root.modArch
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Tray"
                    active: root.modTray
                    onActivated: root.modTray = !root.modTray
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Captures"
                    active: root.modMediaBrowser
                    onActivated: root.modMediaBrowser = !root.modMediaBrowser
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Theme/Wall"
                    active: root.modThemeDisplay
                    onActivated: root.modThemeDisplay = !root.modThemeDisplay
                }
                Tile {
                    width: (col.width - 8) / 2
                    label: "Idle Inhib."
                    active: root.modIdleInhibitor
                    onActivated: root.modIdleInhibitor = !root.modIdleInhibitor
                }
            }
            ...
            // ── BAR STYLE (top-bar layout: default slot bar vs minimal) ──
            Text {
                text: "BAR STYLE"
                color: root.sumi; font.family: root.mono; font.pixelSize: 10; font.letterSpacing: 1
            }
            Row {
                ...
                Repeater {
                    ...
                    delegate: Rectangle {
                        id: bsTile
                        required property var modelData
                        readonly property bool on:      root.barStyle === modelData.mode
                        readonly property bool hovered: bsMa.containsMouse
                        width: (barStyleRow.width - barStyleRow.spacing * (barStyleRow.opts.length - 1)) / barStyleRow.opts.length
                        height: 25; radius: 4
                        color: on ? Qt.rgba(root.seal.r, root.seal.g, root.seal.b, 0.18)
                                  : hovered ? Qt.rgba(root.ink.r, root.ink.g, root.ink.b, 0.12)
                                            : Qt.rgba(root.ink.r, root.ink.g, root.ink.b, 0.06)
                        ...
                        Text {
                            anchors.centerIn: parent
                            text: bsTile.modelData.label
                            color: (bsTile.on || bsTile.hovered) ? root.seal : root.ink
                            font.family: root.mono; font.pixelSize: 10
                            font.weight: bsTile.on ? Font.Medium : Font.Normal
                        }
                        MouseArea {
                            id: bsMa
                            anchors.fill: parent; hoverEnabled: true
                            cursorShape: Qt.PointingHandCursor
                            onClicked: root.barStyle = bsTile.modelData.mode
                        }
                    }
                }
            }
