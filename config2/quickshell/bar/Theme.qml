...
Item {
    ...
    property real barOpacity:  0.94    // igual à waybar (rgba .../0.3)
    ...
    // ── Bar style: which top-bar layout to render. "default" = slot bar (BarSlot),
    //    "minimal" = compact minimal bar (BarMinimal). Persisted in widgets cache.
    property string barStyle: "default"
    onBarStyleChanged: if (_widgetsLoaded) saveWidgets()

    // ── Bar layout / unlock (drag&drop reorder). barUnlocked is transient. ──
    property bool barUnlocked: false
    // ── Bar visibility toggle (SUPER SHIFT Q via IPC). Shared across monitors. ──
    ...
    property bool modClock:        true
    property bool modScreenRecord: true
    property bool modNotification: true
    property bool modMpris:        true
    property bool modArch:         true
    property bool modTray:         true
    property bool modMediaBrowser: true
    property bool modThemeDisplay: true
    property bool modIdleInhibitor: true
    ...
    onModWeatherChanged:       if (_widgetsLoaded) saveWidgets()
    onModBatteryChanged:       if (_widgetsLoaded) saveWidgets()
    onModWorkspaceChanged:     if (_widgetsLoaded) saveWidgets()
    onModClockChanged:         if (_widgetsLoaded) saveWidgets()
    onModScreenRecordChanged:  if (_widgetsLoaded) saveWidgets()
    onModNotificationChanged:  if (_widgetsLoaded) saveWidgets()
    onModMprisChanged:         if (_widgetsLoaded) saveWidgets()
    onModArchChanged:          if (_widgetsLoaded) saveWidgets()
    onModTrayChanged:          if (_widgetsLoaded) saveWidgets()
    onModMediaBrowserChanged:  if (_widgetsLoaded) saveWidgets()
    onModThemeDisplayChanged:  if (_widgetsLoaded) saveWidgets()
    onModIdleInhibitorChanged: if (_widgetsLoaded) saveWidgets()
    ...
    function saveWidgets() {
        var line = (modMemory    ? "1" : "0") + " "
                 ...
                 + (clock12h        ? "1" : "0") + " "
                 + (modNetwork      ? "1" : "0") + " "
                 // newly-appended flags → old caches just keep the property defaults
                 + (modCpu          ? "1" : "0") + " "
                 + (modVolume       ? "1" : "0") + " "
                 + (modWeather      ? "1" : "0") + " "
                 + (modBattery      ? "1" : "0") + " "
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (wsField >= 0 && parts.length > wsField) {
                    ...
                    // newly-appended flags (each guarded → old caches keep defaults)
                    if (parts.length > wsField + 5)  theme.modCpu          = parts[wsField + 5]  === "1"
                    if (parts.length > wsField + 6)  theme.modVolume       = parts[wsField + 6]  === "1"
                    if (parts.length > wsField + 7)  theme.modWeather      = parts[wsField + 7]  === "1"
                    if (parts.length > wsField + 8)  theme.modBattery      = parts[wsField + 8]  === "1"
                    if (parts.length > wsField + 9)  theme.modWorkspace    = parts[wsField + 9]  === "1"
                    if (parts.length > wsField + 10) theme.modClock        = parts[wsField + 10] === "1"
                    if (parts.length > wsField + 11) theme.modScreenRecord = parts[wsField + 11] === "1"
                    if (parts.length > wsField + 12) theme.modNotification = parts[wsField + 12] === "1"
                    if (parts.length > wsField + 13) theme.modMpris        = parts[wsField + 13] === "1"
                    if (parts.length > wsField + 14) theme.modArch         = parts[wsField + 14] === "1"
                    if (parts.length > wsField + 15) theme.modTray         = parts[wsField + 15] === "1"
                    if (parts.length > wsField + 16) theme.modMediaBrowser = parts[wsField + 16] === "1"
                    if (parts.length > wsField + 17) theme.modThemeDisplay  = parts[wsField + 17] === "1"
                    if (parts.length > wsField + 18) theme.modIdleInhibitor = parts[wsField + 18] === "1"
                    if (parts.length > wsField + 19) {
                        var bs = parts[wsField + 19]
                        if (bs === "default" || bs === "minimal") theme.barStyle = bs
                    }
                }
